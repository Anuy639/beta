...
cd /etc/openvpn/
wget https://raw.githubusercontent.com/anuy639/beta/bz/vpn.zip
unzip vpn.zip
rm -f vpn.zip
...
iptables -t nat -I POSTROUTING -s 10.6.0.0/24 -o $ANUY -j MASQUERADE
iptables -t nat -I POSTROUTING -s 10.7.0.0/24 -o $ANUY -j MASQUERADE
